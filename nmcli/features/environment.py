...
def nm_pid():
    try:
        pid = int(
            check_output(["systemctl", "show", "-pMainPID", "NetworkManager.service"])
            .decode("utf-8", "ignore")
            .split("=")[-1]
        )
    ...
    if not pid:
        try:
            pid = int(
                check_output(["pgrep", "NetworkManager"]).decode("utf-8", "ignore")
            )
        except CalledProcessError as e:
            pid = None
    return pid
...
def dump_status_nmtui(fd, when):
    fd.write(
        "Network configuration %s scenario:\n----------------------------------\n"
        % when
    )
    cmds = [
        'date "+%Y%m%d-%H%M%S.%N"',
        ...
        if context.nm_restarted:
            if do_report:
                context.crashed_step = (
                    "crash during scenario (NM restarted, not sure where was the crash)"
                )
        else:
            if do_report:
                context.crashed_step = (
    ...
    for dump_dir in list_of_dumps:
        ...
        if not is_dump_reported(dump_dir):
            ...
            embed_dump(
                context,
                dump_dir,
                dump_output.decode("utf-8", "ignore"),
                "COREDUMP",
                do_report,
            )
...
def setup_libreswan(mode, dh_group, phase1_al="aes", phase2_al=None):
    ...
    RC = call(
        "MODE=%s sh prepare/libreswan.sh > /tmp/libreswan_setup.log" % (mode),
        shell=True,
    )
    if RC != 0:
...
def get_ethernet_devices():
    devs = (
        check_output("nmcli dev | grep ' ethernet' | awk '{print $1}'", shell=True)
        .decode("utf-8", "ignore")
        .strip()
    )
    return devs.split("\n")
...
def reset_hwaddr_nmcli(ifname):
    if not os.path.isfile("/tmp/nm_newveth_configured"):
        hwaddr = (
            check_output("ethtool -P %s" % ifname, shell=True)
            .decode("utf-8", "ignore")
            .split()[2]
        )
...
def setup_hostapd():
    ...
    if arch != "s390x":
        ...
        call(
            "[ -x /usr/sbin/hostapd ] || (yum -y install hostapd; sleep 10)", shell=True
        )
...
def wifi_rescan():
    ...
    out = (
        check_output("time sudo nmcli dev wifi list --rescan yes", shell=True)
        .decode("utf-8", "ignore")
        .strip()
    )
...
def setup_hostapd_wireless():
    ...
    if (
        call("sh prepare/hostapd_wireless.sh tmp/8021x/certs namespace", shell=True)
        != 0
    ):
...
def restore_testeth0():
    ...
    if not os.path.isfile("/tmp/nm_plugin_keyfiles"):
        ...
        call(
            "yes 2>/dev/null | cp -rf /tmp/testeth0 /etc/sysconfig/network-scripts/ifcfg-testeth0",
            shell=True,
        )
    else:
        # defaults to keyfiles (F33+)
        call(
            "yes 2>/dev/null | cp -rf /tmp/testeth0 /etc/NetworkManager/system-connections/testeth0.nmconnection",
            shell=True,
        )
...
def before_all(context):
    ...
    def embed_data(mime_type, data, caption):
        ...
        for formatter in context._runner.formatters:
            if (
                "html" in formatter.name
                and getattr(formatter, "embedding", None) is not None
            ):
                formatter.embedding(
                    mime_type=mime_type, data=non_empty_data, caption=caption
                )
                return True
        return False
    ...
    if IS_NMTUI:
        ...
        try:
            ...
            context.log_cursor = (
                check_output(
                    'journalctl --lines=0 --show-cursor |awk \'/^-- cursor:/ {print "\\"--after-cursor="$NF"\\""; exit}\'',
                    shell=True,
                )
                .decode("utf-8", "ignore")
                .strip()
    ...
    if IS_NMTUI:
        try:
            ...
            if "newveth" in scenario.tags:
                if os.path.isfile("/tmp/nm_newveth_configured"):
                    ...
                    f.write(
                        "INFO: VETH SETUP: this test has been disabled in VETH setup"
                    )
                    f.flush()
                    f.close()
                    sys.exit(77)
            ...
            if "ifcfg-rh" in scenario.tags:
                if (
                    call(
                        "NetworkManager --print-config |grep '^plugins=ifcfg-rh'",
                        shell=True,
                    )
                    != 0
                ):
            ...
            if "simwifi" in scenario.tags:
                ...
                arch = (
                    check_output("uname -p", shell=True)
                    .decode("utf-8", "ignore")
                    .strip()
                )
                if arch != "x86_64":
        ...
        try:
            if not os.path.isfile("/tmp/nm_wifi_configured") and not os.path.isfile(
                ...
                if (
                    call("nmcli device |grep testeth0 |grep ' connected'", shell=True)
                    != 0
                ):
                    call(
            ...
            if "skip_in_centos" in scenario.tags:
                ...
                if (
                    call(
                        "grep -q -e 'CentOS Linux release 8' /etc/redhat-release > /dev/null",
                        shell=True,
                    )
                    == 0
                ):
            ...
            if (
                ...
                if os.path.isfile("/tmp/nm_skip_restarts") or os.path.isfile(
                    "/tmp/nm_skip_STR"
                ):
                    sys.exit(77)
            ...
            if "not_on_aarch64_but_pegas" in scenario.tags:
                ...
                ver = (
                    check_output("uname -r", shell=True)
                    .decode("utf-8", "ignore")
                    .strip()
                )
                if arch == "aarch64":
            ...
            if "gsm" in scenario.tags:
                ...
                if not os.path.isfile("/tmp/usb_hub"):
                    ...
                    def reinitialize_devices():
                        if (
                            call(
                                "systemctl is-active ModemManager  > /dev/null",
                                shell=True,
                            )
                            != 0
                        ):
                            ...
                            timer = 40
                            while (
                                call("nmcli device |grep gsm > /dev/null", shell=True)
                                != 0
                            ):
                                sleep(1)
                                timer -= 1
                    ...
                    while True:
                        ...
                        if lock:
                            if is_lock_old(lock):
                                ...
                                print(
                                    (
                                        "** still locked.. wating %s seconds before next try"
                                        % freq
                                    )
                                )
                                if not initialized:
            ...
            if "connectivity" in scenario.tags:
                ...
                call(
                    "echo '[connectivity]' > /etc/NetworkManager/conf.d/99-connectivity.conf",
                    shell=True,
                )
                if "captive_portal" in scenario.tags:
                    call(
                ...
                call(
                    "echo 'response=OK' >> /etc/NetworkManager/conf.d/99-connectivity.conf",
                    shell=True,
                )
                # Change in interval  would affect connectivity tests and captive portal tests too
                call(
                    "echo 'interval=10' >> /etc/NetworkManager/conf.d/99-connectivity.conf",
                    shell=True,
                )
                reload_NM_service()

            if (
                "shutdown_service_any" in scenario.tags
                or "bridge_manipulation_with_1000_slaves" in scenario.tags
            ):
            ...
            if "not_under_internal_DHCP" in scenario.tags:
                if (
                    call("grep -q Ootpa /etc/redhat-release", shell=True) == 0
                    and call("NetworkManager --print-config|grep dhclient", shell=True)
                    != 0
                ):
                    sys.exit(77)
            ...
            if "need_dispatcher_scripts" in scenario.tags:
                ...
                if os.path.isfile("/tmp/nm-builddir"):
                    ...
                    wait_for_testeth0()
                    call(
                        "yum -y install NetworkManager-config-routing-rules", shell=True
                    )
                reload_NM_service()
            ...
            if "ethernet" in scenario.tags:
                ...
                if (
                    call("nmcli con |grep testeth1", shell=True) == 0
                    or call("nmcli con |grep testeth2", shell=True) == 0
                ):
            ...
            if "logging" in scenario.tags:
                context.loggin_level = (
                    check_output("nmcli -t -f LEVEL general logging", shell=True)
                    .decode("utf-8", "ignore")
                    .strip()
                )
            ...
            if "8021x" in scenario.tags:
                ...
                if arch == "s390x":
                    call(
                        "[ -x /usr/sbin/hostapd ] || (yum -y install 'https://vbenes.fedorapeople.org/NM/hostapd-2.6-7.el7.s390x.rpm'; sleep 10)",
                        shell=True,
                    )
                setup_hostapd()
            ...
            if "tcpreplay" in scenario.tags:
                ...
                call(
                    "[ -x /usr/bin/tcpreplay ] || yum -y install tcpreplay", shell=True
                )
            ...
            if "openvpn" in scenario.tags:
                ...
                call(
                    "[ -x /usr/sbin/openvpn ] || sudo yum -y install openvpn NetworkManager-openvpn",
                    shell=True,
                )
                if call("rpm -q NetworkManager-openvpn", shell=True) != 0:
                    call(
                        "sudo yum -y install NetworkManager-openvpn-1.0.8-1.el7.$(uname -p).rpm",
                        shell=True,
                    )
                    restart_NM_service()
                ...
                call(
                    "[ -x /usr/sbin/openvpn ] || sudo yum -y install https://vbenes.fedorapeople.org/NM/openvpn-2.3.8-1.el7.$(uname -p).rpm\
                                                                      https://vbenes.fedorapeople.org/NM/pkcs11-helper-1.11-3.el7.$(uname -p).rpm",
                    shell=True,
                )
                call(
                    "rpm -q NetworkManager-openvpn || sudo yum -y install https://vbenes.fedorapeople.org/NM/NetworkManager-openvpn-1.0.8-1.el7.$(uname -p).rpm",
                    shell=True,
                )
                reload_NM_service()
            ...
            if "libreswan_ikev1_main" in scenario.tags:
                ...
                wait_for_testeth0()
                call(
                    "rpm -q NetworkManager-libreswan || sudo yum -y install NetworkManager-libreswan",
                    shell=True,
                )
                call("/usr/sbin/ipsec --checknss", shell=True)
                mode = "main"
            ...
            if "preserve_8021x_certs" in scenario.tags:
                print("---------------------------")
                call(
                    "curl -s https://raw.githubusercontent.com/NetworkManager/NetworkManager/master/libnm-core/tests/certs/test-key-and-cert.pem -o /tmp/test_key_and_cert.pem",
                    shell=True,
                )
                call(
                    "curl -s https://raw.githubusercontent.com/NetworkManager/NetworkManager/master/libnm-core/tests/certs/test2_ca_cert.pem -o /tmp/test2_ca_cert.pem",
                    shell=True,
                )
            ...
            if "firewall" in scenario.tags:
                ...
                call(
                    "sudo nmcli con modify testeth0 connection.zone public", shell=True
                )
            ...
            if "restore_hostname" in scenario.tags:
                ...
                context.original_hostname = (
                    check_output("hostname", shell=True)
                    .decode("utf-8", "ignore")
                    .strip()
                )
            ...
            if "slow_team" in scenario.tags:
                ...
                print("---------------------------")
                print(
                    "remove all team packages except NM one and reinstall them with delayed version"
                )
                call(
            ...
            if "openvswitch" in scenario.tags:
                ...
                if (
                    arch == "s390x"
                    and call("grep -q Ootpa /etc/redhat-release", shell=True) != 0
                ):
                    sys.exit(77)
                if call("rpm -q NetworkManager-ovs", shell=True) != 0:
                    ...
                    restart_NM_service()
                if (
                    call("systemctl is-active openvswitch", shell=True) != 0
                    or call(
                        "systemctl status ovs-vswitchd.service |grep -q ERR", shell=True
                    )
                    != 0
                ):
            ...
            if "nmstate_setup" in scenario.tags:
                ...
                call(
                    "echo 0 > /proc/sys/net/ipv6/conf/default/use_tempaddr", shell=True
                )

                # Clone default profile but just ipv4 only"
                print("* Rename testeth0 to ipv4 only nmstate")
                call(
                    'nmcli connection clone "$(nmcli -g NAME con show -a)" nmstate',
                    shell=True,
                )
                call(
            ...
            if "remove_fedora_connection_checker" in scenario.tags:
                print("---------------------------")
                print(
                    "Making sure NetworkManager-config-connectivity-fedora is not installed"
                )
                wait_for_testeth0()
                call(
            ...
            if "no_config_server" in scenario.tags:
                ...
                if call("rpm -q NetworkManager-config-server", shell=True) == 1:
                    ...
                    config_files = (
                        check_output("rpm -ql NetworkManager-config-server", shell=True)
                        .decode("utf-8", "ignore")
                        .strip()
                        .split("\n")
                    )
                    for config_file in config_files:
                        ...
                        if os.path.isfile(config_file):
                            ...
                            call(
                                "sudo mv -f %s %s.off" % (config_file, config_file),
                                shell=True,
                            )
                    reload_NM_service()
            ...
            if "permissive" in scenario.tags:
                ...
                if (
                    check_output("getenforce", shell=True)
                    .decode("utf-8", "ignore")
                    .strip()
                    == "Enforcing"
                ):
            ...
            if "tcpdump" in scenario.tags:
                os.system(
                    "echo '~~~~~~~~~~~~~~~~~~~~~~~~~~ TRAFFIC LOG ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~' > /tmp/network-traffic.log"
                )
                Popen(
            ...
            if context.nm_pid is not None:
                ...
                if (
                    call(
                        "[ -f /etc/systemd/system/NetworkManager.service ] && grep -q valgrind /etc/systemd/system/NetworkManager.service",
                        shell=True,
                    )
                    == 0
                ):
                    call(
                        "LOGNAME=root HOSTNAME=localhost gdb /usr/sbin/NetworkManager -ex 'target remote | vgdb' -ex 'monitor leak_check summary' -batch",
                        shell=True,
                        stdout=context.log,
                        stderr=context.log,
                    )
    ...
    if IS_NMTUI:
        ...
        try:
            ...
            if (
                (
                    step.name
                    == 'Flag "NM_802_11_DEVICE_CAP_AP" is set in WirelessCapabilites'
                    or step.name
                    == 'Flag "NM_802_11_DEVICE_CAP_ADHOC" is set in WirelessCapabilites'
                )
                ...
                f = open("/tmp/tui-screen.log", "a+")
                f.write(
                    "INFO: Skiped the test as device does not support AP/ADHOC mode"
                )
                f.flush()
                f.close()
                sys.exit(77)
        ...
        if (
            step.name
            == 'Flag "NM_802_11_DEVICE_CAP_FREQ_5GHZ" is not set in WirelessCapabilites'
            and step.status == "failed"
            and step.step_type == "given"
        ):
    ...
    if IS_NMTUI:
        ...
        try:
            ...
            if os.path.isfile("/tmp/tui-screen.log"):
                context.embed(
                    "text/plain",
                    utf_only_open_read("/tmp/tui-screen.log"),
                    caption="TUI",
                )
            # Stop TUI
            ...
            if scenario.status == "failed" and hasattr(context, "embed"):
                os.system(
                    "sudo journalctl -u NetworkManager --no-pager -o cat %s > /tmp/journal-session.log"
                    % context.log_cursor
                )
            ...
            if "bond" in scenario.tags:
                os.system(
                    "sudo nmcli connection delete id bond0 bond-slave-eth1 bond-slave-eth2"
                )
                reset_hwaddr_nmtui("eth1")
                reset_hwaddr_nmtui("eth2")
            ...
            if "team" in scenario.tags:
                os.system(
                    "sudo nmcli connection delete id team0 team-slave-eth1 team-slave-eth2"
                )
                reset_hwaddr_nmtui("eth1")
                reset_hwaddr_nmtui("eth2")
            ...
            if "ifcfg-rh" in scenario.tags:
                if (
                    call(
                        "test -f /etc/NetworkManager/conf.d/99-xxcustom.conf",
                        shell=True,
                    )
                    == 0
                ):
            ...
            if "restart" in scenario.tags:
                ...
                if not os.path.isfile("/tmp/nm_dcb_inf_wol_sriov_configured"):
                    wait_for_testeth0()
            if (
                ("ethernet" in scenario.tags)
                or ("ipv4" in scenario.tags)
                or ("ipv6" in scenario.tags)
            ):
                os.system(
                    "sudo nmcli connection delete id ethernet ethernet1 ethernet2"
                )
            ...
            if "nmtui_bridge_add_many_slaves" in scenario.tags:
                os.system(
                    "sudo nmcli con delete bridge-slave-eth3 bridge-slave-eth4 bridge-slave-eth5"
        ...
        try:
            ...
            if "tcpdump" in scenario.tags:
                ...
                if os.stat("/tmp/network-traffic.log").st_size < 20000000:
                    ...
                    print(
                        "WARNING: 20M size exceeded in /tmp/network-traffic.log, skipping"
                    )
            ...
            if "checkpoint_remove" in scenario.tags:
                ...
                if (
                    int(
                        prop_get.Get("org.freedesktop.NetworkManager", "Version").split(
                            "."
                        )[1]
                    )
                    > 10
            ...
            if "restore_hostname" in scenario.tags:
                ...
                call(
                    "hostnamectl set-hostname --static %s" % context.original_hostname,
                    shell=True,
                )
                call("rm -rf /etc/NetworkManager/conf.d/90-hostname.conf", shell=True)
                call("rm -rf /etc/dnsmasq.d/dnsmasq_custom.conf", shell=True)
            ...
            if "con_vrf_remove" in scenario.tags:
                ...
                call(
                    "nmcli connection delete id vrf.eth4 vrf.eth1 vrf0 vrf1", shell=True
                )
            ...
            if "alias" in scenario.tags:
                ...
                call("nmcli connection delete eth7", shell=True)
                call(
                    "sudo rm -f /etc/sysconfig/network-scripts/ifcfg-eth7:0", shell=True
                )
                call(
                    "sudo rm -f /etc/sysconfig/network-scripts/ifcfg-eth7:1", shell=True
                )
                call(
                    "sudo rm -f /etc/sysconfig/network-scripts/ifcfg-eth7:2", shell=True
                )
            ...
            if "gen-bond_remove" in scenario.tags:
                ...
                call(
                    "nmcli connection delete id gen-bond0 gen-bond0.0 gen-bond0.1",
                    shell=True,
                )
            ...
            if "connectivity" in scenario.tags:
                ...
                call(
                    "rm -rf /etc/NetworkManager/conf.d/99-connectivity.conf", shell=True
                )
                call(
            ...
            if "remove_tombed_connections" in scenario.tags:
                ...
                for dir in [
                    ...
                    try:
                        tombs.extend(
                            check_output("ls %s" % dir, shell=True)
                            .decode("utf-8", "ignore")
                            .split("\n")
                        )
                    except:
            ...
            if "logging" in scenario.tags:
                ...
                call(
                    "sudo nmcli g log level %s domains ALL" % context.loggin_level,
                    shell=True,
                )
            ...
            if "eth0" in scenario.tags:
                ...
                if "restore_hostname" in scenario.tags:
                    call('hostnamectl set-hostname --transien ""', shell=True)
                    call(
                        "hostnamectl set-hostname --static %s"
                        % context.original_hostname,
                        shell=True,
                    )
                wait_for_testeth0()
            ...
            if "mtu" in scenario.tags:
                ...
                call(
                    "nmcli connection modify testeth1 802-3-ethernet.mtu 1500",
                    shell=True,
                )
                call("nmcli connection up id testeth1", shell=True)
                call(
                    "nmcli connection modify testeth1 802-3-ethernet.mtu 0", shell=True
                )
            ...
            if "mtu_wlan0" in scenario.tags:
                ...
                call(
                    "nmcli connection modify qe-open 802-11-wireless.mtu 1500",
                    shell=True,
                )
            ...
            if "macsec" in scenario.tags:
                print("---------------------------")
                call(
                    "sudo nmcli connection delete test-macsec test-macsec-base",
                    shell=True,
                )
            ...
            if "two_bridged_veths6" in scenario.tags:
                ...
                print("deleting veth devices")
                call(
                    "nmcli connection delete id tc16 tc26 test10 test11 vethbr6",
                    shell=True,
                )
            ...
            if "sriov" in scenario.tags:
                ...
                call(
                    "set 1 to /sys/class/net/p4p1/device/sriov_drivers_autoprobe",
                    shell=True,
                )
                call(
                    "echo 1 > /sys/class/net/p4p1/device/sriov_drivers_autoprobe",
                    shell=True,
                )
                call(
                    "echo 1 > /sys/class/net/p6p1/device/sriov_drivers_autoprobe",
                    shell=True,
                )
            ...
            if (
                "simwifi_teardown" in scenario.tags
                or "simwifi_teardown_doc" in scenario.tags
            ):
                print("---------------------------")
            ...
            if "simwifi_p2p" in scenario.tags:
                ...
                call(
                    "kill -9 $(ps aux|grep wpa_suppli |grep wlan1 |awk '{print $2}')",
                    shell=True,
                )
            ...
            if "simwifi_ap" in scenario.tags:
                ...
                call(
                    "nmcli con del wifi-ap wifi-client br0 br0-slave1 br0-slave2",
                    shell=True,
                )
            ...
            if "dracut" in scenario.tags:
                ...
                if scenario.status == "failed" and os.path.isfile(
                    "/tmp/dracut_boot.log"
                ):
                ...
                if os.path.isfile("/tmp/dracut_setup.log"):
                    ...
                    context.embed(
                        "text/plain",
                        utf_only_open_read("/tmp/dracut_setup.log"),
                        "DRACUT_SETUP",
                    )
                ...
                if os.path.isfile("/tmp/dracut_clean.log"):
                    ...
                    context.embed(
                        "text/plain",
                        utf_only_open_read("/tmp/dracut_clean.log"),
                        "DRACUT_CLEAN",
                    )
                    call("rm -f /tmp/dracut_clean.log", shell=True)
                call(
                    "journalctl -all --no-pager %s | grep ' dhcpd\\[' > /tmp/journal-dhcpd.log"
                    % context.log_cursor,
                    shell=True,
                )
                context.embed(
                    "text/plain", utf_only_open_read("/tmp/journal-dhcpd.log"), "DHCPD"
                )
                call(
                    "journalctl -all --no-pager %s | grep ' radvd\\[' > /tmp/journal-radvd.log"
                    % context.log_cursor,
                    shell=True,
                )
                context.embed(
                    "text/plain", utf_only_open_read("/tmp/journal-radvd.log"), "RADVD"
                )
                call(
                    "journalctl -all --no-pager %s | grep ' rpc.mountd\[' > /tmp/journal-nfs.log"
                    % context.log_cursor,
                    shell=True,
                )
                context.embed(
                    "text/plain", utf_only_open_read("/tmp/journal-nfs.log"), "NFS"
                )
            ...
            if "ethernet" in scenario.tags:
                ...
                call(
                    "sudo nmcli connection delete id ethernet ethernet0 ethos",
                    shell=True,
                )
                call(
            ...
            if "con_general_remove" in scenario.tags:
                ...
                print("removing ethernet profiles")
                call(
                    "sudo nmcli connection delete id con_general con_general2",
                    shell=True,
                )
            ...
            if "scapy" in scenario.tags:
                ...
                call("ip link delete test11", shell=True)
                call(
                    "nmcli connection delete ethernet-test10 ethernet-test11",
                    shell=True,
                )
            ...
            if "keyfile_cleanup" in scenario.tags:
                print("---------------------------")
                print(
                    "removing residual files in /usr/lib/NetworkManager/system-connections"
                )
                call(
                    'sudo sh -c "rm /usr/lib/NetworkManager/system-connections/*" ',
                    shell=True,
                )
                print(
                    "removing residual files in /etc/NetworkManager/system-connections"
                )
                call(
                    'sudo sh -c "rm /etc/NetworkManager/system-connections/*" ',
                    shell=True,
                )
            ...
            if "remove_dns_clean" in scenario.tags:
                if (
                    call("grep dns /etc/NetworkManager/NetworkManager.conf", shell=True)
                    == 0
                ):
                    call(
                        "sudo sed -i 's/dns=none//' /etc/NetworkManager/NetworkManager.conf",
                        shell=True,
                    )
                call(
                    "sudo rm -rf /etc/NetworkManager/conf.d/90-test-dns-none.conf; sleep 1",
                    shell=True,
                )
                reload_NM_service()
                ...
                if context.restore_config_server:
                    ...
                    for config_file in config_files:
                        ...
                        if os.path.isfile(config_file + ".off"):
                            ...
                            call(
                                "sudo mv -f %s.off %s" % (config_file, config_file),
                                shell=True,
                            )
                    reload_NM_service()
                call(
            ...
            if "openvswitch" in scenario.tags:
                ...
                call(
                    "sudo nmcli con del eth1 eth2 ovs-bond0 ovs-port0 ovs-patch0 ovs-patch1 ovs-bridge1 ovs-bridge0 ovs-port1 ovs-eth2 ovs-eth3 ovs-iface0 eth2 dpdk-sriov c-ovs-br0 c-ovs-port0 c-ovs-iface0",
                    shell=True,
                )  # to be sure
                sleep(1)
                call("ovs-vsctl del-br ovsbr0", shell=True)
                ...
                call(
                    "sudo rm -rf /etc/sysconfig/network-scripts/ifcfg-eth1", shell=True
                )
                call(
                    "sudo rm -rf /etc/sysconfig/network-scripts/ifcfg-bond0", shell=True
                )
                call(
            ...
            if "remove_custom_cfg" in scenario.tags:
                ...
                call(
                    "sudo rm -f /etc/NetworkManager/conf.d/99-xxcustom.conf", shell=True
                )
                restart_NM_service()

            if (
                "device_connect_no_profile" in scenario.tags
                or "device_connect" in scenario.tags
            ):
                print("---------------------------")
            ...
            if "nmcli_general_keep_slave_device_unmanaged" in scenario.tags:
                print("---------------------------")
                print(
                    "restoring the testeth8 profile to managed state / removing slave"
                )
                call("sudo ip link del eth8.100", shell=True)
                call(
                    "sudo rm -f /etc/sysconfig/network-scripts/ifcfg-testeth8",
                    shell=True,
                )
                call("sudo nmcli connection reload", shell=True)
                call(
            ...
            if "nmcli_general_profile_pickup_doesnt_break_network" in scenario.tags:
                ...
                context.nm_restarted = True
                call(
                    "sudo nmcli connection delete con_general con_general2", shell=True
                )
            ...
            if "add_testeth10" in scenario.tags:
                ...
                call(
                    "sudo nmcli connection add type ethernet con-name testeth10 ifname eth10 autoconnect no",
                    shell=True,
                )
            ...
            if "add_testeth1" in scenario.tags:
                ...
                call(
                    "sudo nmcli connection add type ethernet con-name testeth1 ifname eth1 autoconnect no",
                    shell=True,
                )
            ...
            if "add_testeth5" in scenario.tags:
                ...
                call(
                    "sudo nmcli connection add type ethernet con-name testeth5 ifname eth5 autoconnect no",
                    shell=True,
                )
            ...
            if "add_testeth8" in scenario.tags:
                ...
                call(
                    "sudo nmcli connection add type ethernet con-name testeth8 ifname eth8 autoconnect no",
                    shell=True,
                )
            ...
            if "need_dispatcher_scripts" in scenario.tags:
                ...
                call(
                    "rm -rf /etc/sysconfig/network-scripts/rule-con_general", shell=True
                )
                call(
            ...
            if "iptunnel_doc" in scenario.tags:
                ...
                call(
                    "nmcli con delete gre1 tun0 bridge0 bridge0-port1 bridge0-port2",
                    shell=True,
                )
            ...
            if scenario.status == "failed" or context.crashed_step:
                ...
                os.system(
                    "echo '~~~~~~~~~~~~~~~~~~~~~~~~~~ NM LOG ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~' > /tmp/journal-nm.log"
                )
                os.system(
                    "sudo journalctl -all -u NetworkManager --no-pager -o cat %s >> /tmp/journal-nm.log"
                    % context.log_cursor
                )
            ...
            if nm_pid_after is not None and context.nm_pid == nm_pid_after:
                ...
                if (
                    ...
                    call(
                        "LOGNAME=root HOSTNAME=localhost gdb /usr/sbin/NetworkManager -ex 'target remote | vgdb' -ex 'monitor leak_check full kinds all increased' -batch",
                        shell=True,
                        stdout=context.log,
                        stderr=context.log,
                    )
            ...
            context.embed(
                "text/plain",
                utf_only_open_read("/tmp/log_%s.html" % scenario.name),
                caption="MAIN",
            )
            ...
            if context.crashed_step:
                ...
                if not context.crash_embeded:
                    context.embed(
                        "text/plain",
                        "!!! no crash report detected, but NM PID changed !!!",
                        caption="NO_COREDUMP/NO_FAF",
                    )
