...
Feature: nmcli - ethernet
    ...
    * Note the output of "ethtool eno2 |grep Wake-on |grep Supports | awk '{print $3}'" as value "wol_supports"
    * Note the output of "ethtool eno2 |grep Wake-on |grep -v Supports | awk '{print $2}'" as value "wol_orig"
    * Restart NM
    * Add connection type "ethernet" named "ethernet" for device "eno2"
    ...
    * Bring up connection "ethernet"
    * Note the output of "ethtool eno2 |grep Wake-on |grep -v Supports | awk '{print $2}'" as value "wol_now"
    ...
    * Bring up connection "ethernet"
    Then "Wake-on: g" is visible with command "ethtool eno2"
    ...
    * Bring up connection "ethernet"
    Then "Wake-on: d" is visible with command "ethtool eno2"
    ...
    * Bring up connection "ethernet"
    Then "ETHTOOL_OPTS" is not visible with command "cat /etc/sysconfig/network-scripts/ifcfg-ethernet"
    * Note the output of "ethtool eno2 |grep Wake-on |grep -v Supports | awk '{print $2}'" as value "wol_new"
