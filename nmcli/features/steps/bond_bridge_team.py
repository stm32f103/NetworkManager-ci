...
def check_bond_in_proc(context, bond):
    child = pexpect.spawn(
        "cat /proc/net/bonding/%s " % (bond), logfile=context.log, encoding="utf-8"
    )
...
def check_slave_in_team_is_up(context, slave, team, state):
    ...
    if state == "up":
        if r != 0:
            ...
            if r != 0:
                raise Exception(
                    "Device %s was not found in dump of team %s" % (slave, team)
                )
    ...
    if state == "down":
        if r == 0:
            ...
            if r == 0:
                raise Exception(
                    "Device %s was found in dump of team %s" % (slave, team)
                )
...
def check_slave_not_in_bond_in_proc(context, slave, bond):
    ...
    assert (
        child.expect(["Slave Interface: %s\s+MII Status: up" % slave, pexpect.EOF]) != 0
...
def team_is_down(context, team):
    ...
    if nmci_step.command_code(context, "teamdctl %s state dump" % team) == 0:
        time.sleep(1)
        assert (
            nmci_step.command_code(context, "teamdctl %s state dump" % team) != 0
        ), 'team "%s" exists' % (team)
