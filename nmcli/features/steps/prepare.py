...
def create_policy_based_routing_files(context, profile, dev, table):
    ips = nmci_step.command_output(
        context, "nmcli connection sh %s |grep IP4.ADDRESS |awk '{print $2}'" % profile
    )
    ip_slash_prefix = ips.split("\n")[0]
    ip = ip_slash_prefix.split("/")[0]
    gw = nmci_step.command_output(
        context, "nmcli connection sh %s |grep IP4.GATEWAY |awk '{print $2}'" % profile
    ).strip()
    nmci_step.command_code(
        context,
...
def prepare_sriov_config(context, conf, device, vfs):
    ...
    nmci_step.command_code(
        context, "echo 'match-device=interface-name:%s' >> %s" % (device, conf_path)
    )
    nmci_step.command_code(
        context, "echo 'sriov-num-vfs=%d' >> %s" % (int(vfs), conf_path)
    )
...
def prepare_pppoe_server(context, user, passwd, ip, auth):
    ...
    nmci_step.command_code(
        context, "echo '%s * %s %s' > /etc/ppp/%s-secrets" % (user, passwd, ip, auth)
    )
...
def prepare_veths(context, pairs_array, bridge):
    ...
    for pair in pairs:
        nmci_step.command_code(
            context, "ip link add %s type veth peer name %sp" % (pair, pair)
        )
...
def restart_dhcp_server(context, device, ipv4, ipv6):
    nmci_step.command_code(
        context, "kill $(cat /tmp/{device}_ns.pid)".format(device=device)
    )
    nmci_step.command_code(
        context,
...
def prepare_dhcpd_simdev(context, device, server_id):
    ...
    nmci_step.command_code(
        context, "ip link set {device}p netns {device}_ns".format(device=device)
    )
    nmci_step.command_code(context, "ip link set {device} up".format(device=device))
    nmci_step.command_code(
        context, "ip netns exec {device}_ns ip link set lo up".format(device=device)
    )
    nmci_step.command_code(
        context,
    ...
    nmci_step.command_code(
        context, "echo '192.168.99.10 ip-192-168-99-10' >> /etc/hosts"
    )
    nmci_step.command_code(
        context, "echo '192.168.99.11 ip-192-168-99-11' >> /etc/hosts"
    )
    nmci_step.command_code(
        context, "echo '192.168.99.12 ip-192-168-99-12' >> /etc/hosts"
    )
    nmci_step.command_code(
        context, "echo '192.168.99.13 ip-192-168-99-13' >> /etc/hosts"
    )
    nmci_step.command_code(
        context, "echo '192.168.99.14 ip-192-168-99-14' >> /etc/hosts"
    )
    nmci_step.command_code(
        context, "echo '192.168.99.15 ip-192-168-99-15' >> /etc/hosts"
    )
...
def prepare_simdev(
    ...
    assert (
        nmci_step.command_code(context, dnsmasq_command) == 0
...
def prepare_simdev(context, device):
    ...
    nmci_step.command_code(
        context, "ip link set {device}2 netns {device}_ns".format(device=device)
    )
    nmci_step.command_code(
        context, "ip link set {device}2p netns {device}2_ns".format(device=device)
    )
    # Bring up devices
    nmci_step.command_code(context, "ip link set {device} up".format(device=device))
    nmci_step.command_code(
...
def prepare_simdev(context, device):
    ...
    subprocess.Popen(
        "ip netns exec {device}2_ns nc -6 -l -p 8080 > /dev/null".format(device=device),
        shell=True,
    )
...
def setup_macsec_psk(context, cak, ckn):
    ...
    nmci_step.command_code(
        context, "ip link add macsec_veth type veth peer name macsec_vethp"
    )
    ...
    nmci_step.command_code(
        context, "ip netns exec macsec_ns ip link set macsec_vethp up"
    )
    ...
    nmci_step.command_code(
        context, "echo '  key_mgmt=NONE' >> /tmp/wpa_supplicant.conf"
    )
    nmci_step.command_code(
        context, "echo '  eapol_flags=0' >> /tmp/wpa_supplicant.conf"
    )
    nmci_step.command_code(
        context, "echo '  macsec_policy=1' >> /tmp/wpa_supplicant.conf"
    )
    nmci_step.command_code(
        context, "echo '  mka_cak={cak}' >> /tmp/wpa_supplicant.conf".format(cak=cak)
    )
    nmci_step.command_code(
        context, "echo '  mka_ckn={ckn}' >> /tmp/wpa_supplicant.conf".format(ckn=ckn)
    )
    ...
    time.sleep(6)
    assert (
        nmci_step.command_code(context, "ip netns exec macsec_ns ip link show macsec0")
        == 0
    ...
    nmci_step.command_code(
        context, "ip netns exec macsec_ns ip addr add 172.16.10.1/24 dev macsec0"
    )
    nmci_step.command_code(
        context,
...
def prepare_iptunnel_doc(context, mode):
    ...
    if bridge:
        ...
        nmci_step.command_code(
            context, "ip -n iptunnelB address add 172.16.0.1/24 dev netB"
        )
    ...
    nmci_step.command_code(
        context, "ip -n iptunnelB address add 198.51.100.5/32 dev ipB"
    )
    nmci_step.command_code(context, "ip -n iptunnelB route add 203.0.113.10/32 dev ipB")
    assert (
        nmci_step.command_code(context, "ping -c 1 198.51.100.5") == 0
    ), "unable to ping public IP of B from A"
    assert (
        nmci_step.command_code(
            context, "ip netns exec iptunnelB ping -c 1 203.0.113.10"
        )
        == 0
    ...
    if not bridge:
        ...
        nmci_step.command_code(
            context, "ip -n iptunnelB route add 192.0.2.0/24 dev tunB"
        )
    else:
