...
def check_dif_in_values(context, i2, i1, dif):
    assert (int(context.noted[i1].strip()) + int(dif)) > int(
        ...
        "Noted values: %s + %s !> %s !"
        % (context.noted[i1].strip(), dif, context.noted[i2].strip())
    )
...
def execute_multiple_times(context, command, number):
    orig_nm_pid = subprocess.check_output("pidof NetworkManager", shell=True).decode(
        "utf-8", "ignore"
    )
    ...
    while i < int(number):
        nmci_step.command_code(context, command)
        curr_nm_pid = subprocess.check_output(
            "pidof NetworkManager", shell=True
        ).decode("utf-8", "ignore")
        assert curr_nm_pid == orig_nm_pid, (
            "NM crashed as original pid was %s but now is %s"
            % (orig_nm_pid, curr_nm_pid,)
        )
        i += 1
...
def get_nameserver_or_domain(context, server, seconds=1):
    if (
        subprocess.call("systemctl is-active systemd-resolved.service -q", shell=True)
        == 0
    ):
...
def note_print_property_b(context, pattern):
    assert re.search(pattern, context.noted["noted-value"]) is not None, (
        ...
        % (context.noted["noted-value"], pattern)
    )
...
def note_the_output_as(context, command, index):
    if not hasattr(context, "noted"):
        context.noted = {}
    context.noted[index] = nmci_step.command_output(
        context, command + " 2>/dev/null"
    ).strip()
    ...
    while seconds > 0:
        ...
        if check_type == "default":
            ...
            assert ret == 0, (
                ...
                % (pattern, orig_seconds - seconds, proc.before)
            )
        elif check_type == "not_full":
            assert ret != 0, (
                'Pattern "%s" appeared after %d seconds, output was:\n%s%s'
                % (pattern, orig_seconds - seconds, proc.before, proc.after)
            )
        seconds = seconds - 1
        time.sleep(interval)
    if check_type == "default":
        raise Exception(
            'Did not see the pattern "%s" in %d seconds, output was:\n%s'
            % (pattern, orig_seconds, proc.before)
        )
    elif check_type == "not":
        raise Exception(
            'Did still see the pattern "%s" in %d seconds, output was:\n%s%s'
            % (pattern, orig_seconds, proc.before, proc.after)
        )
    return True
...
def run_child_process_no_shell(context, command):
    children = getattr(context, "children", [])
    children.append(
        subprocess.Popen(command.split(" "), stdout=context.log, stderr=context.log)
    )
    context.children = children
...
def start_tailing_journal(context):
    context.journal = pexpect.spawn(
        "sudo journalctl --follow -o cat",
        timeout=180,
        logfile=context.log,
        encoding="utf-8",
    )
...
def start_tailing(context, archivo):
    context.tail = pexpect.spawn(
        "sudo tail -f %s" % archivo, timeout=180, logfile=context.log, encoding="utf-8"
    )
    time.sleep(0.3)
...
def ping_domain(context, domain, number=2):
    if number != 2:
        ping = pexpect.spawn(
            "ping -q -4 -c %s %s" % (number, domain),
            logfile=context.log,
            encoding="utf-8",
        )
    else:
        ping = pexpect.spawn(
            "curl -s %s" % (domain), logfile=context.log, encoding="utf-8"
        )
...
def ping_domain_from_device(context, domain, device):
    ping = pexpect.spawn(
        "ping -4 -c 2 -I %s %s" % (device, domain),
        logfile=context.log,
        encoding="utf-8",
    )
    ping.expect([pexpect.EOF])
...
def ping6_domain(context, domain):
    ping = pexpect.spawn(
        "ping6 -c 2 %s" % domain, logfile=context.log, encoding="utf-8"
    )
    ping.expect([pexpect.EOF])
    ping.close()
    assert ping.exitstatus == 0, "msg: %s" % ping.before
...
def cannot_ping_domain_from_device(context, domain, device):
    ping = pexpect.spawn(
        "ping -c 2 -I %s %s " % (device, domain), logfile=context.log, encoding="utf-8"
    )
    ping.expect([pexpect.EOF])
    ping.close()
    assert ping.exitstatus != 0
...
def network_dropped_two(context, state, device):
    if state == "is":
        assert (
            nmci_step.command_code(context, "ping -c 2 -I %s -W 1 8.8.8.8" % device)
            != 0
        )
    if state == "is not":
        assert (
...
def set_logging(context, domain, level):
    if level == " ":
        ...
        cli = pexpect.spawn(
            "nmcli g l level %s domains %s" % (level, domain),
            timeout=60,
            logfile=context.log,
            encoding="utf-8",
        )
...
def note_NM_log(context):
    if not hasattr(context, "noted"):
        context.noted = {}
    context.noted["noted-value"] = subprocess.check_output(
