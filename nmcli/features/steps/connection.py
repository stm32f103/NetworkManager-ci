...
def add_vpnc_connection_for_iface(context, name, ifname, vpn):
    ...
    if r == 0:
        raise Exception(
            "Got an Error while adding %s connection %s for device %s\n%s%s"
            % (vpn, name, ifname, cli.after, cli.buffer)
        )
...
def add_connection_for_iface(context, typ, name, ifname):
    ...
    if r == 0:
        raise Exception(
            "Got an Error while adding %s connection %s for device %s\n%s%s"
            % (typ, name, ifname, cli.after, cli.buffer)
        )
...
def add_new_default_connection_without_ifname(context, typ, options):
    cli = pexpect.spawn(
        "nmcli connection add type %s %s" % (typ, options),
        logfile=context.log,
        encoding="utf-8",
    )
    if cli.expect(["Error", pexpect.TIMEOUT, pexpect.EOF]) == 0:
        raise Exception(
            ...
            % (typ, options, cli.after, cli.buffer)
        )
...
def modify_connection_with_noted(context, connection, prop):
    ...
    if r == 0:
        raise Exception(
            ...
            % (prop, connection, context.noted["noted-value"], cli.after, cli.buffer)
        )
...
def start_stop_connection(context, action, name):
    if action == "down":
        if (
            nmci_step.command_code(
                context, "nmcli connection show --active |grep %s" % name
            )
            != 0
        ):
    ...
    cli = pexpect.spawn(
        "nmcli connection %s id %s" % (action, name),
        logfile=context.log,
        timeout=180,
        encoding="utf-8",
    )
    ...
    if r == 0:
        raise Exception(
            "Got an Error while %sing connection %s\n%s%s"
            % (action, name, cli.after, cli.buffer)
        )
    elif r == 1:
...
def start_connection_for_device(context, name, device):
    cli = pexpect.spawn(
        "nmcli connection up id %s ifname %s" % (name, device),
        logfile=context.log,
        timeout=180,
        encoding="utf-8",
    )
    ...
    if r == 0:
        ...
        raise Exception(
            "Got an Error while uping connection %s on %s\n%s%s"
            % (name, device, cli.after, cli.buffer)
        )
    elif r == 1:
...
def bring_up_connection(context, connection):
    cli = pexpect.spawn(
        "nmcli connection up %s" % connection,
        timeout=180,
        logfile=context.log,
        encoding="utf-8",
    )
    ...
    if r == 0:
        raise Exception(
            "Got an Error while upping connection %s\n%s%s"
            % (connection, cli.after, cli.buffer)
        )
    elif r == 1:
...
def bring_down_connection(context, connection):
    cli = pexpect.spawn(
        "nmcli connection down %s" % connection,
        timeout=180,
        logfile=context.log,
        encoding="utf-8",
    )
    ...
    if r == 0:
        raise Exception(
            "Got an Error while downing a connection %s\n%s%s"
            % (connection, cli.after, cli.buffer)
        )
    elif r == 1:
...
def delete_connection(context, connection):
    cli = pexpect.spawn(
        "nmcli connection delete %s" % connection,
        timeout=95,
        logfile=context.log,
        encoding="utf-8",
    )
    ...
    if res == 0:
        raise Exception(
            "Got an Error while deleting connection %s\n%s%s"
            % (connection, cli.after, cli.buffer)
        )
    elif res == 1:
...
def modify_connection(context, name, options):
    out = nmci_step.command_output(
        context, "nmcli connection modify %s %s" % (name, options)
    )
    if "Error" in out:
        raise Exception(
            "Got an Error while modifying %s options %s\n%s" % (name, options, out)
        )
...
def start_generic_connection(context, connection, device):
    cli = pexpect.spawn(
        "nmcli connection up %s ifname %s" % (connection, device),
        timeout=180,
        logfile=context.log,
        encoding="utf-8",
    )
