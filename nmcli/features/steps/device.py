...
def add_secondary_addr_same_subnet(context, device):
    ...
    primary_ipn = IPNetwork(
        nmci_step.command_output(
            context, "ip -4 a s %s | awk '/inet .*dynamic/ {print $2}'" % device
        )
    )
...
def create_device_in_ns(context, dev, ns, addr):
    nmci_step.command_code(
        context, "ip -n %s link add %s type veth peer name %sp" % (ns, dev, dev)
    )
...
@step("Compare kernel and NM devices")
...
def connect_device(context, device):
    ...
    if r == 0:
        raise Exception(
            "Got an Error while connecting a device %s\n%s%s"
            % (device, cli.after, cli.buffer)
        )
    elif r == 1:
...
def connect_wifi_device(context, network):
    cli = pexpect.spawn(
        'nmcli device wifi connect "%s"' % network,
        timeout=180,
        logfile=context.log,
        encoding="utf-8",
    )
...
def connect_wifi_device_w_options(context, network, options):
    cli = pexpect.spawn(
        'nmcli device wifi connect "%s" %s' % (network, options),
        timeout=180,
        logfile=context.log,
        encoding="utf-8",
    )
...
def note_mac_address(context, device):
    if not hasattr(context, "noted"):
        context.noted = {}
    context.noted["noted-value"] = nmci_step.command_output(
        context,
...
def note_mac_address_ip(context, device, index=None):
    if subprocess.call("ip a s %s |grep -q ether" % device, shell=True) == 0:
        mac = nmci_step.command_output(
            context, "ip link show %s | grep 'link/ether' | awk '{print $2}'" % device
        ).strip()
    if subprocess.call("ip a s %s |grep -q infiniband" % device, shell=True) == 0:
        ip_out = nmci_step.command_output(
            context, "ip link show %s | grep 'link/inf' | awk '{print $2}'" % device
        ).strip()
        mac = ip_out.split()[-1]
        client_id = ""
...
def disconnect_connection(context, name):
    cli = pexpect.spawn(
        "nmcli device disconnect %s" % name,
        logfile=context.log,
        timeout=180,
        encoding="utf-8",
    )
    ...
    if r == 0:
        raise Exception(
            "Got an Error while disconnecting device %s\n%s%s"
...
def delete_device(context, device):
    ...
    if r == 0:
        raise Exception(
            "Got an Error while deleting device %s\n%s%s"
            % (device, cli.after, cli.buffer)
        )
    elif r == 1:
...
def check_lldp_neighbours(context, property, device):
    ...
    for prop in property.split(","):
        ...
        queue = path.split(":")
        bad_vals = []
        assert check_obj(queue[0], lldp_neighbors, val, queue[1:]), (
            "value '%s' for property '%s' not found in ['%s']"
            % (val, path, "','".join(bad_vals))
        )
...
def force_renew_ipv6(context, device):
    mac = nmci_step.command_output(
        context, "ip a s %s |grep fe80 |awk '{print $2}'" % device
    ).strip()
...
def check_ipv6_connectivity_on_assumal(context, profile, device):
    ...
    address = nmci_step.command_output(
        context,
        "ip -6 a s %s | grep dynamic | awk '{print $2; exit}' | cut -d '/' -f1"
        % device,
    )
    ...
    assert (
        nmci_step.command_code(
            context,
            "sed -i 's/UUID=/#UUID=/' /etc/sysconfig/network-scripts/ifcfg-%s"
            % profile,
        )
        == 0
    )
    ping = pexpect.spawn(
        "ping6 %s -i 0.2 -c 50" % address, logfile=context.log, encoding="utf-8"
    )
    time.sleep(1)
    assert (
        nmci_step.command_code(context, "systemctl start NetworkManager.service") == 0
    )
    time.sleep(12)
