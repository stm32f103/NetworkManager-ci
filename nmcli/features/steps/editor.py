...
def open_editor_for_connection(context, con_name):
    ...
    prompt = pexpect.spawn(
        "/bin/nmcli connection ed %s" % con_name, logfile=context.log, encoding="utf-8"
    )
    context.prompt = prompt
    r = prompt.expect([con_name, "Error"])
    if r == 1:
        raise Exception(
            "Got an Error while opening profile %s\n%s%s"
            % (con_name, prompt.after, prompt.buffer)
        )
...
def open_editor_for_connection_with_timeout(context, con_name):
    prompt = pexpect.spawn(
        "nmcli connection ed %s" % (con_name),
        logfile=context.log,
        maxread=6000,
        timeout=5,
        encoding="utf-8",
...
def open_editor_for_connection_type(context, con_name, type):
    prompt = pexpect.spawn(
        "nmcli connection ed type %s con-name %s" % (type, con_name),
        logfile=context.log,
        maxread=6000,
        encoding="utf-8",
    )
    ...
    if r != 0:
        raise Exception(
            "Got an Error while opening  %s profile %s\n%s%s"
            % (type, con_name, prompt.after, prompt.buffer)
        )
...
def open_editor_for_new_connection(context):
    prompt = pexpect.spawn(
        "nmcli connection edit", logfile=context.log, encoding="utf-8"
    )
    context.prompt = prompt
...
def open_interactive_for_a_type(context, typ):
    prompt = pexpect.spawn(
        "nmcli -a connection add type %s" % typ,
        timeout=5,
        logfile=context.log,
        encoding="utf-8",
    )
...
def open_interactive(context):
    prompt = pexpect.spawn(
        "nmcli -a connection add", timeout=5, logfile=context.log, encoding="utf-8"
    )
    context.prompt = prompt
...
def add_novice_connection(context):
    prompt = pexpect.spawn(
        "nmcli -a connection add", logfile=context.log, encoding="utf-8"
    )
    context.prompt = prompt
